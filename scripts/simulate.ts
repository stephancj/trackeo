...
/**
 * simulate_tana.ts — Simulateur de mouvement GPS pour Trackeo (Antananarivo)
 *
 * Envoie des requêtes HTTP GET au protocole OsmAnd de Traccar (port 5055)
 * Trajet : Analakely (Arcades) -> Ankorondrano (Zone Galaxy)
 */
...
const CONFIG = {
  traccarHost: process.env.TRACCAR_HOST ?? 'localhost',
  traccarPort: parseInt(process.env.TRACCAR_PORT ?? '5055', 10),
  deviceId: process.env.DEVICE_ID ?? 'tana-taxi-001',
  intervalMs: parseInt(process.env.INTERVAL_MS ?? '3000', 10),
  totalPoints: parseInt(process.env.TOTAL_POINTS ?? '40', 10),
  altitudeMeters: '1280', // Altitude moyenne d'Antananarivo
};

// ─── Route simulée (Analakely -> Antanimena -> Ankorondrano) ──────────────────
// Coordonnées réelles pour Tana (Latitude Sud est négative)
const ROUTE_WAYPOINTS: [number, number][] = [
  [-18.9102, 47.5255],   // Départ : Arcades Analakely
  [-18.9070, 47.5250],   // Avenue de l'Indépendance
  [-18.9035, 47.5235],   // Montée Antanimena (EESAE)
  [-18.9010, 47.5262],   // Carrefour Antanimena (Zone rouge embouteillages)
  [-18.8970, 47.5255],   // Route des Hydrocarbures (Ankorondrano entrée)
  [-18.8920, 47.5265],   // Intersection Boulevard de l'Europe
  [-18.8885, 47.5285],   // Devant Jumbo / Score Ankorondrano
  [-18.8845, 47.5302],   // Arrivée : Zone Galaxy
];
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      console.log(
        `[${index.toString().padStart(3, '0')}] ✓ HTTP ${res.statusCode} | ` +
        `Lat: ${lat.toFixed(5)}, Lon: ${lon.toFixed(5)} | ` +
        `Vitesse: ${speed.toFixed(1)} km/h | Cap: ${bearing.toFixed(0)}°`,
      );
      resolve();
    });

    req.on('error', (err) => {
      console.error(`[${index}] ✗ Erreur de connexion : ${err.message}`);
      reject(err);
    });
...
async function main(): Promise<void> {
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  Trackeo GPS Simulator - Antananarivo');
  console.log(`  Véhicule : ${CONFIG.deviceId}`);
  console.log(`  Trajet   : Analakely -> Ankorondrano`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  for (let i = 0; i < route.length; i++) {
    ...
    // --- Simulation de vitesse réaliste (Facteur Tana) ---
    let speed = 25 + Math.random() * 15; // Vitesse de base urbaine

    // Ralentissement à Antanimena (Zone de forte congestion entre -18.905 et -18.898)
    if (current[0] > -18.905 && current[0] < -18.898) {
      speed = 5 + Math.random() * 8; // On simule un bouchon (5-13 km/h)
    }

    // Vitesse nulle si c'est le dernier point
    if (i === route.length - 1) speed = 0;
    ...
    try {
      ...
      console.warn(`[${i + 1}] Traccar injoignable, vérifiez l'IP et le port.`);
    }

    if (i < route.length - 1) {
      // Ajout d'un petit délai aléatoire pour simuler l'irrégularité du trafic
      const jitter = Math.random() * 1000;
      await new Promise((r) => setTimeout(r, CONFIG.intervalMs + jitter));
    }
  }

  console.log('\n✅ Simulation terminée. Le véhicule est arrivé à destination.');
}
